...
if response.status_code == 200:
    ...
    for v in voice_list:
        if isinstance(v, dict):
            # Try to find 'IN' in any string value
            if any(isinstance(val, str) and "IN" in val.upper() for val in v.values()):
                print(f"ID: {v.get('voiceId') or v.get('voice_id')}, Name: {v.get('displayName') or v.get('name')}, Locale: {v.get('locale')}")
else:
